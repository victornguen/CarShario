...
import carshario.data.models.Car
import carshario.data.types.{CarManufacturer, CarModel, Latitude, Longitude, VIN}
...
class CarStorageImpl(config: String)(implicit ec: ExecutionContext) extends CarStorage[IO] {
  ...
  override def add(car: Car): IO[Either[Throwable, Int]] = {
    ...
    val res = mayBeCar.flatMap {
      case Some(value) => Future.successful(Left(new Exception)) //TODO: define exceptions
